...
it("traverses HTTP -> D1 -> Workflow -> HTTP Integration -> D1, and reuses a submission", async () => {
  ...
  expect((await worker.fetch(request("/api/executions", "POST", "changed"), bindings)).status).toBe(409);
  expect((await worker.fetch(request(`/api/executions/${id}`), { ...bindings, LAB_USER_ID: "00000000-0000-4000-8000-000000000003" })).status).toBe(404);
  expect((await worker.fetch(request(`/api/executions/${id}`), { ...bindings, LAB_ORG_ID: "00000000-0000-4000-8000-000000000004" })).status).toBe(404);
...
it("does not resolve another Organization's Connection when this Organization has none", async () => {
  ...
  await bindings.DB.prepare("INSERT INTO connections(id,org_id,integration_id,endpoint) VALUES ('other','other-organization','720b9ebf-9b6a-4eac-bae9-6ed22c970402','http://127.0.0.1:8788/echo')").run();
  const id = await executionId(principal, key);
  await using instance = await introspectWorkflowInstance(bindings.ECHO_WORKFLOW, id);
  expect((await worker.fetch(request("/api/executions", "POST"), bindings)).status).toBe(202);
  await instance.waitForStatus("errored");

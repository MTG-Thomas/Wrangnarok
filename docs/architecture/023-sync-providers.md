...
| Timeout | Slow vendor (past 5000ms) answers `504 PROVIDER_TIMEOUT` with the Pending receipt; no invented success, no late overwrite. |
...
| Scope denial | Foreign org/user Connection rows never resolve; unknown Saga IDs 404; `org_id`/`run_as` fields 400. |
| Output size | Persisted results stay under the 4096-byte D1 bound; oversized vendor shapes answer `413 PROVIDER_OUTPUT_TOO_LARGE` with no row write. |

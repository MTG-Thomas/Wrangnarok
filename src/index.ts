...
async function handleFetch(request: Request, env: Bindings): Promise<Response> {
  ...
  try {
    ...
    // file structural list, and the file byte routes take them, and only
    // their allowlisted keys (anything else is UNSUPPORTED_QUERY).
    ...
    if (url.search && !historyList && !fileList && !fileBytes)

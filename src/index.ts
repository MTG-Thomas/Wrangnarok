...
import { boundedJson, echoSaga, Fault, parseKey, parseSubmission } from "./domain";
import { submit, summary, visibleJourney } from "./journeys";
import type { JourneyRow } from "./journeys";
export { EchoWorkflow } from "./sagas";
...
export default {
  async fetch(request: Request, env: Bindings): Promise<Response> {
    try {
      ...
      if (url.pathname === "/api/sagas" && request.method === "GET") return json({ sagas: [echoSaga] });
      if (url.pathname === "/api/journeys" && request.method === "POST") {
        ...
        const accepted = await submit(env, caller, key, parseSubmission(await boundedJson(request.body)));
      ...
      if (url.pathname === "/api/journeys" && request.method === "GET") {
        ...
        const rows = await env.DB.prepare("SELECT id,saga_id,saga_name,saga_revision,grove_id,user_id,dispatched,status,created_at,started_at,completed_at FROM journeys WHERE grove_id=? AND user_id=? ORDER BY created_at DESC,id DESC LIMIT 21")
          .bind(caller.groveId, caller.userId).all<Omit<JourneyRow, "input_json" | "result_json" | "error_json">>();
        return json({ journeys: rows.results.slice(0, 20).map(summary), hasMore: rows.results.length > 20 });
      }
      const match = /^\/api\/journeys\/([a-f0-9]{64})$/.exec(url.pathname);
      if (match?.[1] && request.method === "GET") {
        const row = await visibleJourney(env.DB, match[1], caller);
        const operations = await env.DB.prepare("SELECT name,status,started_at,completed_at,result_json,error_json FROM operations WHERE journey_id=? ORDER BY position,name")
        ...
        try { runtimeStatus = (await (await env.ECHO_WORKFLOW.get(row.id)).status()).status; } catch { /* Unavailable or expired, not proof of failure. */ }

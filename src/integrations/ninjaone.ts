...
import { boundedJson, Fault, NINJA_INTEGRATION_ID, NINJA_ORGS_MAX, NINJA_ORGS_PATH, NINJA_SCOPE, NINJA_TOKEN_PATH } from "../domain";
...
export async function listOrganizations(
  ...
  if (response.status >= 300 && response.status < 400) { await response.body?.cancel(); throw new Fault(502, "NINJA_VENDOR_FAILED", "NinjaOne redirected the request."); }
...
async function fetchToken(connection: NinjaConnection, credentials: NinjaCredentials): Promise<string> {
  ...
  if (response.status >= 300 && response.status < 400) { await response.body?.cancel(); throw new Fault(502, "NINJA_AUTH_FAILED", "NinjaOne redirected the token request."); }
  if (response.status === 401) { await response.body?.cancel(); throw new Fault(502, "NINJA_UNAUTHORIZED", "NinjaOne rejected the credentials."); }

...
export async function echo(connection: EchoConnection, input: EchoInput, operationId: string, timeoutMs = VENDOR_TIMEOUT_MS): Promise<EchoInput> {
  ...
  try {
    ...
    if (timedOut() || (error instanceof DOMException && (error.name === "TimeoutError" || error.name === "AbortError"))) {

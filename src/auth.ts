...
/** Local fixture only. Grove and user never come from request headers or JSON. */
export async function authenticate(request: Request, env: LabAuth): Promise<Principal> {
  ...
  if (!env.LAB_TOKEN || !/^[a-f0-9]{64}$/.test(env.LAB_TOKEN) ||
      !env.LAB_GROVE_ID || !UUID.test(env.LAB_GROVE_ID) ||
  ...
  return { userId: env.LAB_USER_ID.toLowerCase(), groveId: env.LAB_GROVE_ID.toLowerCase() };
}

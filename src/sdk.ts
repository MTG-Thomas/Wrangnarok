...
export function describeContract(): SdkContractDescriptor {
  return {
    ...
    routes: [
      ...
      { method: "GET", path: "/api/apps/:id/grants", description: "List scoped app grants (author)." },
      { method: "POST", path: "/api/apps/:id/grants", description: "Create a scoped app grant (author)." },
      {
        method: "POST",
        path: "/api/apps/:id/grants/:grantId/revoke",
        description: "Revoke a scoped app grant (author).",
      },
      {
        method: "GET",
        path: "/api/apps/:id/tables",
        description: "List declared app Tables (author, includes hidden).",
      },
      { method: "POST", path: "/api/apps/:id/tables", description: "Declare an app Table (author)." },
      { method: "GET", path: "/api/apps/:id/sdk", description: "App SDK handshake descriptor (version tripwire)." },
      {
        method: "GET",
        path: "/api/apps/:id/runtime/tables",
        description: "List visible app Tables (runtime, granted).",
      },
      {
        method: "GET",
        path: "/api/apps/:id/runtime/tables/:name/rows",
        description: "Filtered Table page read (runtime, read grant; filter/limit/cursor/sinceRevision).",
      },
      {
        method: "POST",
        path: "/api/apps/:id/runtime/tables/:name/rows",
        description: "Insert one Table row (runtime, write grant).",
      },
      {
        method: "PATCH",
        path: "/api/apps/:id/runtime/tables/:name/rows/:rowId",
        description: "Replace one Table row (runtime, write grant).",
      },
      {
        method: "DELETE",
        path: "/api/apps/:id/runtime/tables/:name/rows/:rowId",
        description: "Delete one Table row (runtime, write grant).",
      },
      {
        method: "POST",
        path: "/api/apps/:id/runtime/invoke",
        description: "Invoke a granted Saga (runtime, invoke grant; Idempotency-Key required).",
      },
      {
        method: "GET",
        path: "/api/apps/:id/runtime/executions",
        description: "Scoped invocation activity tail (runtime; result via GET /api/executions/:id).",
      },
      { method: "GET", path: "/api/apps/:id/runtime/files", description: "List read-granted files (runtime)." },
      {
        method: "POST",
        path: "/api/apps/:id/runtime/files",
        description: "Declare a file location (runtime, write grant).",
      },
      {
        method: "POST",
        path: "/api/apps/:id/runtime/files/tokens",
        description: "Issue a single-use upload/download token (runtime, matching grant).",
      },
      {
        method: "POST",
        path: "/api/apps/:id/runtime/files/upload",
        description: "Redeem an upload token with finalize-after-upload verification (runtime).",
      },
      {
        method: "POST",
        path: "/api/apps/:id/runtime/files/download",
        description: "Redeem a download token (runtime; ready files only).",
      },
      {
        method: "DELETE",
        path: "/api/apps/:id/runtime/files/*",
        description: "Delete a file version-aware (runtime, write grant; expectedVersion query).",
      },
    ],
    ...
    capabilities: [
      ...
      },
      {
        ...
        status: "supported",
        detail:
          "Scoped browser App SDK runtime (ADR 018): grant-scoped Saga invoke, visible-Table read/write with revision polling, versioned files with single-use tokens, handshake tripwire. No WebSocket; no Forms/config hooks.",
      },
      {
        name: "local-preview",
        status: "supported",

...
export function describeContract(): SdkContractDescriptor {
  return {
    ...
    routes: [
      ...
      { method: "GET", path: "/api/apps", description: "Application summaries for this Organization (APP-01)." },
      { method: "POST", path: "/api/apps", description: "Create an independent app (no draft/publish step)." },
      {
        method: "GET",
        path: "/api/apps/:id",
        description: "App detail with revisions, deploy jobs, and the active deployment.",
      },
      { method: "DELETE", path: "/api/apps/:id", description: "Delete an independent app (owned rows refuse)." },
      {
        method: "PUT",
        path: "/api/apps/:id/source",
        description: "Edit source declarations (independent only; MANAGED_RESOURCE on owned rows).",
      },
      {
        method: "POST",
        path: "/api/apps/:id/validate",
        description: "Validate the current revision (422 on failure).",
      },
      {
        method: "POST",
        path: "/api/apps/:id/builds",
        description: "Start an async deploy job (validate-gated; failed builds preserve the live app).",
      },
      { method: "GET", path: "/api/apps/:id/builds", description: "Inspect the async deploy-job queue." },
      { method: "GET", path: "/api/apps/:id/builds/:jobId", description: "Inspect one deploy job." },
      {
        method: "POST",
        path: "/api/apps/:id/swap",
        description: "Parked-old-app slug-swap recovery (fenced on both slugs).",
      },
      {
        method: "GET",
        path: "/api/apps/:id/assets/*",
        description: "Serve one file from the active deployment (same Organization; no-store + ETag).",
      },
    ],
    ...
    capabilities: [
      ...
      { name: "execute-status-cancel", status: "supported", detail: "Submit, poll, detail, history, and cancel." },
      {
        ...
        status: "supported",
        detail:
          "Independent apps: create, edit, validate, build (async deploy job), inspect jobs, slug-swap recovery, delete, and authorized active-deployment asset serving (ADR 016). Solution-owned rows reject live mutation; no draft/publish step and no retained-history rollback UI.",
      },
      {

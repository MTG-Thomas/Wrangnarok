...
function Test-GoalTransition {
  <#.SYNOPSIS Seen-file gate: true only on first-seen-complete, new blockers,
  or first completion. Everything else (progress) stays silent. #>
  param(
  ...
  if (-not $SeenPath) {
    $SeenPath = Join-Path ([System.IO.Path]::GetTempPath()) 'opencode\goal-seen.json'
  }
  $seen = New-Object psobject
  if (Test-Path -LiteralPath $SeenPath) {
    $seen = Get-Content -LiteralPath $SeenPath -Raw | ConvertFrom-Json
  }
  $getEntry = {
    param($id)
    $p = $seen.PSObject.Properties[$id]
    if ($null -eq $p) { return $null }
    return $p.Value
  }
  $setEntry = {
    param($id, $entry)
    if ($null -ne $seen.PSObject.Properties[$id]) { $seen.PSObject.Properties.Remove($id) }
    $seen | Add-Member -NotePropertyName $id -NotePropertyValue $entry
  }
  $prev = & $getEntry $GoalId
  $announce = $false
  $kind = ''
  $detail = ''
  if ($null -eq $prev) {
    $entry = [pscustomobject]@{
      status = $Status; announcedComplete = $false; announcedBlocked = @()
    }
    if ($Status -eq 'complete') {
      $announce = $true; $kind = 'complete'; $detail = 'Goal already complete on first sight.'
      $entry.announcedComplete = $true; $entry.announcedBlocked = @($Blocked)
    } else {
      $entry.announcedBlocked = @($Blocked)
    }
    & $setEntry $GoalId $entry
  } else {
    $prevBlocked = @($prev.announcedBlocked)
    $fresh = @($Blocked | Where-Object { $prevBlocked -notcontains $_ })
    if ($fresh.Count -gt 0) {
      $announce = $true; $kind = 'blocked'; $detail = ($fresh -join '; ')
    }
    $done = [bool]$prev.announcedComplete
    if ($Status -eq 'complete' -and -not $done) {
      $announce = $true; $kind = 'complete'; $detail = 'Goal marked complete.'
      $done = $true
    }
    & $setEntry $GoalId ([pscustomobject]@{
      status = $Status; announcedComplete = $done
      announcedBlocked = @(@($prevBlocked) + @($Blocked) | Select-Object -Unique)
    })
  }
  $seen | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $SeenPath
  return [pscustomobject]@{ Announce = $announce; Kind = $kind; Detail = $detail }
}

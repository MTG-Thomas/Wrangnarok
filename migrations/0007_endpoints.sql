CREATE TABLE endpoints(
  ...
  kind TEXT NOT NULL CHECK(kind IN ('api-key','webhook')),
  enabled INTEGER NOT NULL DEFAULT 1 CHECK(enabled IN (0,1)),
  key_hash TEXT,
  key_expires_at TEXT,
  signature_secret_hash TEXT,
  challenge TEXT NOT NULL DEFAULT 'none' CHECK(challenge IN ('none','echo-param')),
  rate_limit_per_minute INTEGER CHECK(rate_limit_per_minute IS NULL OR rate_limit_per_minute > 0),
  created_at TEXT NOT NULL,
  UNIQUE(org_id, name)
);
CREATE INDEX endpoints_org ON endpoints(org_id);

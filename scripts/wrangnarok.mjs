...
Commands:
  ...
  cleanup [--run]                         Preview (or, with --run, execute*) cleanup
                                          * admin only: X-Wrangnarok-Admin + ADMIN_USER_ID
    ...
    case "retention": {
      ...
      return readJson(
        await fetchImpl(`${ctx.base}/api/artifacts/retention`, {
          method: "PUT",
          headers: { ...full.headers, ...(ctx.admin ? { "X-Wrangnarok-Admin": "true" } : {}) },
...
async function main() {
  ...
  const ctx = {
    ...
    admin: command === "retention" || command === "cleanup" ? flag("admin") : false,
  };
...
async function selftest() {
  ...
  // retention reads by default and sets with --days; cleanup previews and runs.
  {
    ...
    await runCommand(
      { ...base, command: "retention", retentionDays: 7, admin: true },
      { fetchImpl: setStub.fetch, ...noSleep },
    );
    check("retention admin header", setStub.calls[0].init.headers["X-Wrangnarok-Admin"] === "true");
